            ...
            <div class="col-md-12">
                ...
                <div class="review-map">
                    <ul class="nav review-tabs" role="tablist">
                        @php
                            $locations = ['Hà Nội', 'Hồ Chí Minh', 'Quảng Ninh', 'Bắc Ninh', 'Bắc Giang'];
                        @endphp
                        @foreach($locations as $index => $location)
                        <li>
                            <button class="{{ $index == 0 ? 'active' : '' }}" data-bs-toggle="tab" data-bs-target="#map-type-{{ $index + 1 }}" type="button">{{ $location }}</button>
                        </li>
                        @endforeach
                    </ul>
                    <div class="tab-content">
                        @foreach($locations as $index => $location)
                        <div class="tab-pane fade {{ $index == 0 ? 'active show' : '' }}" id="map-type-{{ $index + 1 }}">
                            <div class="row">
