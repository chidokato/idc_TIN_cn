<div class="card card-s card-s4 ">
    <a href="{{$val->category->slug}}/{{$val->slug}}">
        ...
        <span class="cat">
            {!! $val->monopoly?'<span class="room-status bg-red">独家</span>':'' !!}
            <!-- {!! $val->for_sale?'<span class="room-status bg-red">正在销售</span>':'' !!} -->
            {!! $val->new_product?'<span class="room-status">新推出</span>':'' !!}
        </span>
        ...
        <div class="product-status">
            ...
            <span> {!! $val->bedroom ? '<i class="icon-bed me-1"></i>'.$val->bedroom.($val->bedroom_max ? ' - '.$val->bedroom_max : '').' 卧室' : '' !!}</span>
            <span> {!! $val->wc ? '<i class="icon-bathroom me-1"></i>'.$val->wc.($val->wc_max ? ' - '.$val->wc_max : '').' 卫浴' : '' !!}</span>
        </div>
    </a>
    <div class="card-body">
        ...
        <div class="d-flex card-body-price">
            <div class="card-price">
                价格: <span class="current-price">
                    {{$val->price >= 1000000000?$val->price/1000000000 . ' B': ($val->price? $val->price/1000000 . ' M':'联系咨询') }}
                    {{$val->price_max >= 1000000000? ' - ' . $val->price_max/1000000000 . ' B':($val->price_max?' - ' . $val->price_max/1000000 . ' M':'')}}
                </span>
            </div>  
            <div>{{ $val->total_product? $val->total_product.'套':'' }}</div>
        </div>
    </div>
</div>
